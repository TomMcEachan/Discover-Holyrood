...
describe('Test Page Title', () => {
    ...
    it('Page Subtitle is correct', () => {
        cy.visit(`${url}/learn`)
        cy.get('#page-subtitle').should('have.text', 'Find out about the Scottish Parliament')
    })
})
...
describe('Test Search Functionality', () => {
    ...
    it ('Faceting instructions are visible', () => {
        cy.visit(`${url}/learn`)
        cy.get('#facet-instructions').should('be.visible').should('have.text', 'Tap a category to filter learning material:')
    })

    it('Current Refinements instructions are visible', () => {
        cy.visit(`${url}/learn`)
        cy.get('#facet-filters-note').should('be.visible').should('have.text', 'No filters are currently applied')
    })

    it('Current Refinements are visible', () => {
        cy.visit(`${url}/learn`)
        cy.get('#facet-container').children().eq(0).click()
        cy.get('#facet-filters-note').should('be.visible').should('have.text', 'Currently Applied Filters:')
        cy.get('#current-refinements').should('be.visible')
        cy.get('#current-refinements').children().should('have.length', 1)
        cy.get('#current-refinements').children().eq(0).should('have.text', 'Building')
    })
    ...
    it ('Facets have correct text', () => {
        ...
        cy.get('#facet-container').children().eq(0).should('have.text', 'Building')
        cy.get('#facet-container').children().eq(1).should('have.text', 'Chamber')
        cy.get('#facet-container').children().eq(2).should('have.text', 'History')
        cy.get('#facet-container').children().eq(3).should('have.text', 'Laws')
        cy.get('#facet-container').children().eq(4).should('have.text', 'MSPs')
        cy.get('#facet-container').children().eq(5).should('have.text', 'Powers')
    })
    ...
    it('Facets return correct results', () => {
        ...
        cy.get('#search-result').children().eq(1).children().should('have.text', 'Building')
    })
    ...
    it ('Search bar has right placeholder text', () => {
        cy.visit(`${url}/learn`)
        cy.get('#search-bar-input').should('have.attr', 'placeholder', 'Search for learning material...')
    })
